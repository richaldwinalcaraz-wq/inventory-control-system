  ...
  if (order.status === "DRAFT" && currentUser.role === "SALES_REP") {
    cards.push(
      <ActionCard key="confirm" title="Confirm order">
        <button disabled={pending} onClick={() => run(() => postJson(`/api/v1/sales-orders/${order.id}/confirm`, {}))} className="rounded-md bg-slate-900 px-4 py-2 text-sm font-medium text-white hover:bg-slate-800 disabled:opacity-50">
          Confirm
        </button>
      </ActionCard>,
    );
  ...
  if (order.status === "CONFIRMED" && currentUser.role === "WAREHOUSE_SUPERVISOR") {
    cards.push(
      <ActionCard key="reserve" title="Reserve stock">
        <button disabled={pending} onClick={() => run(() => postJson(`/api/v1/sales-orders/${order.id}/reserve`, {}))} className="rounded-md bg-slate-900 px-4 py-2 text-sm font-medium text-white hover:bg-slate-800 disabled:opacity-50">
          Reserve
        </button>
      </ActionCard>,
    );
    ...
    if (currentUser.role === "WAREHOUSE_SUPERVISOR") {
      cards.push(
        <ActionCard key="authorize" title="Authorize release">
          <button disabled={pending} onClick={() => run(() => postJson(`/api/v1/sales-orders/${order.id}/authorize-release`, {}))} className="rounded-md bg-slate-900 px-4 py-2 text-sm font-medium text-white hover:bg-slate-800 disabled:opacity-50">
            Authorize
          </button>
        </ActionCard>,
      );
  ...
  if (canVoid) {
    cards.push(
      <ActionCard key="void" title="Void this order">
        <input value={voidReason} onChange={(e) => setVoidReason(e.target.value)} placeholder="Reason" className="mb-2 w-full rounded-md border border-slate-300 px-3 py-2 text-sm" />
        <button
