  ...
  return (
    <form onSubmit={submit} className="flex flex-col gap-4">
      <div>
        ...
        <select value={supplierId} onChange={(e) => setSupplierId(e.target.value)} required className="w-full rounded-md border border-slate-300 px-3 py-2 focus:border-brand-600 focus:outline-none">
          {suppliers.map((s) => (
      ...
      <div>
        ...
        <input value={drNumber} onChange={(e) => setDrNumber(e.target.value)} required className="w-full rounded-md border border-slate-300 px-3 py-2 focus:border-brand-600 focus:outline-none" />
      </div>

      <div>
        <label className="mb-1 block text-sm font-medium text-slate-700">PO reference (optional)</label>
        <input value={poReference} onChange={(e) => setPoReference(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2 focus:border-brand-600 focus:outline-none" />
      ...
      <div className="flex flex-col gap-3">
        ...
        <button type="button" onClick={addLine} className="self-start text-sm text-brand-700 hover:underline">
          + Add another line
        </button>
      </div>
