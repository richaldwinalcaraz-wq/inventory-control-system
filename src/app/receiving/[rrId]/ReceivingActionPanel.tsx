    ...
    cards.push(
      <ActionCard key="tiebreak" title="Step 5 — Tie-break (counts disagreed)">
        <div className="mb-3">
          ...
          <select value={witnessedBy} onChange={(e) => setWitnessedBy(e.target.value)} className="w-full rounded-md border border-slate-300 px-2 py-1.5 text-sm">
  ...
  if (rr.status === "QUARANTINE") {
    cards.push(
      <ActionCard key="quarantine" title="Quarantined">
        <p className="text-sm text-slate-600">Goods were rejected at inspection. Resolution is a manual/Phase 3 process from here.</p>
      </ActionCard>,
    );
  }
  if (rr.status === "VOID") {
