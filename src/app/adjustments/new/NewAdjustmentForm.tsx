  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch("/api/v1/adjustments", {
        ...
        body: JSON.stringify({ productVariantId, warehouseLocationId, reasonCode, quantityDelta, reconciliationNotes }),
      });
  ...
  return (
    <form onSubmit={submit} className="flex flex-col gap-4">
      <div>
        ...
        <select value={productVariantId} onChange={(e) => setProductVariantId(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2">
          {variants.map((v) => (
      ...
      <div>
        ...
        <select value={warehouseLocationId} onChange={(e) => setWarehouseLocationId(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2">
          {locations.map((l) => (
      ...
      <div>
        ...
        <select value={reasonCode} onChange={(e) => setReasonCode(e.target.value)} className="w-full rounded-md border border-slate-300 px-3 py-2">
          {REASON_CODES.map((r) => (
